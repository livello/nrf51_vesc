...
#define GEAR_DRIVE_TIMEOUT 2000
// Private variables
...
static enum ret_codes gear_waiting_low(void) {
...
static enum ret_codes gear_waiting_high(void) {
...
static void gear_drive_stop();

static void gear_drive_fall();

static enum ret_codes gear_falling(void) {
...
static void gear_drive_rise();

static enum ret_codes gear_rising(void) {
    if (gear_drive_time_stop <= now_millis) {
        gear_drive_stop();
        return done;
    }
...
static int (*gear_state_functions[])(void) = {gear_waiting_low, gear_waiting_high, gear_falling, gear_rising};
